...
| 모듈 | 스택 |
|------|------|
| Crawler | Python 3.11, Playwright 1.44, PyMySQL 1.1, schedule 1.2 |
...
| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/posts` | 게시물 목록 (platform, page, size, sort 파라미터 지원) |
| GET | `/api/posts/{postId}` | 게시물 단건 조회 |

Spring Data JPA 기반, Service 레이어 없이 Controller → Repository 직접 호출 구조입니다.
...
| TikTok / X 크롤러 | 미구현 |
| AI 요약 (Claude) | 미구현 |
| Frontend 실제 API 연동 | 부분 구현 (PostListPage만) |
